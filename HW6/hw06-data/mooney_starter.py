...
class Mooney(object):
	...
	def compute_projected_data_matrix(self,X_proj):
		...
		for x in self.x_train:
			...
			# STANDARDIZE DATA
			x_f = standardized(x_f)
			x_f = self.ss_x.transform(x_f)
			# TODO: PROJECT DATA
			X.append(x_f @ X_proj.T)
		...
		for x in self.x_test:
			...
			# STANDARDIZE DATA
			x_f = standardized(x_f)
			y_f = self.ss_y.transform(y_f)
			# TODO: PROJECT DATA
			X_test.append(x_f @ X_proj.T)
	...
	def solve_for_variance(self):
		...
		# TODO: 
		D1 = self.C_xx+self.lmbda*np.eye(675)
		D1 = inv(sqrtm(D1))
		#D1[np.diag_indices(675)] = 1/(D1.diagonal()**0.5)

		D2 = self.C_yy+self.lmbda*np.eye(675)
		D2 = inv(sqrtm(D2))
		#D2[np.diag_indices(675)] = 1/(D2.diagonal()**0.5)

		matrix = D1 @ self.C_xy @ D2

		eigen_vectors, eigen_values, dummy = svd(matrix)
	...
	def ridge_regression(self):
		...
		for i in range(self.y_dim):
			...
			y = self.Y_ridge[:, i]
			w = inv(self.X_ridge.T @ self.X_ridge + self.lmbda*np.eye(self.X_ridge.shape[1])) @ self.X_ridge.T @ y
			w_ridge.append(w)
	...
	def draw_images(self):
		for count, x in enumerate(self.X_test_ridge):
			...
			prediction = self.ss_y.inverse_transform(prediction)
...
if __name__ == '__main__':
	...
	plt.plot(np.arange(675), eig_val)
	plt.xlabel('i')
	plt.ylabel('eigenvalue')
	plt.savefig('spectrum.png')
	#plt.show()

	#u0 = mooney.ss_x.inverse_transform(eig_vec[:, 0])
	#mooney.plot_image(u0)
	...
	X_proj = mooney.project_data(eig_val,eig_vec, proj=100)
	mooney.compute_projected_data_matrix(X_proj)
	mooney.ridge_regression()
	mooney.draw_images()
	#training_error = mooney.measure_error(mooney.X_ridge, mooney.Y_ridge)
