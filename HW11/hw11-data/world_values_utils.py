...
def from_index_get_country(indices):
    """
    Given an array of indices, get a series of country name

    Args:
        indices: an array of indices

    Returns: 
        country_list: a series of country name
    """
    values_train = pd.read_csv('world-values-train2.csv')
    return values_train.iloc[indices]['Country']
...
def calculate_correlations(training_features,
    ...
    plot_hdi_vs_feature(training_features, training_labels, 'Protecting forests rivers and oceans',
                        'green', 'HDI versus Protecting forests rivers and oceans')
    ...
    plot_hdi_vs_feature(training_features, training_labels, 'Access to clean water and sanitation',
                        'green', 'HDI versus Access to clean water and sanitation')
