...
from world_values_utils import plot_pca, from_index_get_country
...
def main():
    ...
    # Part e,f,and g, j, l, n: Regression Grid Searches #
    
    #regression_grid_searches(training_features=values_train,training_labels=hdi_train, testing_features=values_test)

    # Part i, m, : Nearest neighbors to the US. n_neighbors is 8 because the first entry is the US.
    ##scale the data first
    # scaler = StandardScaler()
    # values_train = pd.DataFrame(scaler.fit_transform(values_train))
    # ##
    # nbrs = NearestNeighbors(n_neighbors=8).fit(values_train)
    # distances, indices = nbrs.kneighbors(values_train.iloc[45].values.reshape(1, -1))
    # indices = indices.reshape(-1)
    # print(from_index_get_country(indices))
    # print(distances)

    # Part  t, u, v, w,  Classification Grid Searches #
