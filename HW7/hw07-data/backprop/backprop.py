...
class GDOptimizer(object):
    ...
    def update(self, layers, g, a):
        ...
        for layer, curGrad, curA in zip(layers, g, a):
            ...
            # ######################################################################################
            layer.updateWeights(-self.eta/m * curGrad @ curA.T)
            layer.updateBias(-self.eta/m * np.sum(curGrad, axis = 1).reshape(layer.b.shape))
...
class Model(object):
    ...
    def compute_grad(self, x, y):
        ...
        gradx = [d]  ####to store dMSE / da_i
        grad = []  ####to store dMSE / dz_i
        ...
        for layer, curZ in zip(reversed(self.layers),reversed(z)):
            ...
            grad.append(gradx[-1] * layer.dx(curZ))  ###dMSE /dz_i
            #print(d.shape)
            #print(curZ.shape)
            #print(layer.W)
            gradx.append(layer.W.T @ grad[-1])
