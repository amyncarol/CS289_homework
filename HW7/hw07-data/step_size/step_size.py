...
def main():
	...
	b = np.array([1000, 1000])  # b in the equation ||Ax-b||
	initial_position = np.array([0, 0])  # position at iteration 0
	total_step_count = 100000   # number of GD steps to take
	step_size = lambda i: 1.0/(i+1)  # step size at iteration i
	################################################################################
	...
	plt.title('A = {}, b = {}, reciprocal'.format(A, b))
	plt.plot()
	plt.savefig('tmp.png')
	plt.show()
